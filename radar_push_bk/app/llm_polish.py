...
_SYSTEM_PROMPT = (
    ...
    "3b. 监测实况中的来源只能中性描述为相关区域\"有对流云团正在发展/活动\"，"
...
def _validate(
    ...
    if not _multiset_le(_extract_numbers(template_text), _extract_numbers(text)):
        log.warning("LLM 润色丢失/改动了数字，回退模板\n初稿=%s\n输出=%s", template_text, text)
        return None

    tmpl_dangerous = set(_DANGEROUS_VALUE_RE.findall(template_text))
    out_dangerous = set(_DANGEROUS_VALUE_RE.findall(text))
    extra = out_dangerous - tmpl_dangerous
    if extra:
        log.warning("LLM 润色引入了模板没有的量值 %s，回退模板", extra)
        return None
