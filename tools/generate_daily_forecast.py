"""
title: 当日预报生成器
description: 按发布时段（08/11/17）从「北京市气象台天气公报」+「北京市气象台 240h 精细化预报产品」自动生成某区当日预报 Word 文档
...
【运行依赖】
  - python-docx（读模板/写文档）
  - LibreOffice（soffice）：用于把公报 .doc 转 .docx，Dockerfile 已装
  - 模板目录需包含 morning.docx / noon.docx / afternoon.docx 三个文件

【本地测试小贴士】
  在 open-webui Tools 设置里把 Valves 改成：
    xml_source_mode        = local
    xml_dir                = 你的 240h 精细化预报产品 XML 目录
    bulletin_source_mode   = local
    bulletin_local_dir     = 你的天气公报 .doc 目录
    template_dir           = 你的 daily 模板目录
    soffice_bin            = soffice 或 LibreOffice 的绝对路径
"""

# 数据源官方称呼常量：所有面向用户/模型的文案必须使用这两个常量，
# 严禁在临时拼字符串里写「公报」「240 预报」「BJ-240」等简称作为对外措辞。
...
# ============================================================================
# slot 规约：每天严格 3 个版本，HHMM 与模板/发布时间/段落规则一一对应
# ============================================================================
#
# 设计原则（重要）：
#   公报「二、未来一周天气预报」章节后面的段落顺序是**固定的**——
#     0600 版：第1段 = 今天白天，第2段 = 今天夜间
#     1100 版：第1段 = 今天下午（有时写作「白天」），第2段 = 今天夜间
#     1700 版：第1段 = 今天夜间（有时写作「傍晚-夜间」），第2段 = 明天白天
#   不论公报里这两段的标题被人工写成什么字样（白天/下午/夜间/傍晚-夜间…），
#   它们在章节里的**位置和含义**都是固定的。因此本工具不再尝试按
#   「X日{白天/夜间}」严格匹配，而是直接取章节后的第1、2段，按模板渲染。
#
# 字段说明：
#   - template/publish_hour：渲染时使用
#   - title_p1/title_p2：模板里第1/2段的标题文字（如「今天白天：」）
#   - p1_temp_hour/p2_temp_hour：BJ-240 XML 里取哪个 hour 的温度
#   - temp_label_p1/temp_label_p2：模板里温度行的前缀（最高气温/最低气温）
...
class Tools:
    ...
    async def generate_daily_forecast(
        ...
        数据来源固定为「北京市气象台天气公报」和「北京市气象台 240h 精细化预报产品」，
