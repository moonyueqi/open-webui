"""
title: 未来一周天气预报生成工具
description: 根据「北京市气象台天气公报」+「北京市气象台未来240h预报产品」自动生成某区未来一周天气预报 Word 文档
author: lyq
version: 1.0.0
"""
...
class Tools:
    class Valves(BaseModel):
        ...
        template_path: str = Field(
            default=os.environ.get(
                "WEATHER_TEMPLATE_WEEKLY",
                "/app/weather_templates/weekly/template.docx",
            ),
            description="未来一周天气预报 docx 模板的绝对路径",
        )
        ...
        summary_timeout: int = Field(
            default=int(os.environ.get("WEEKLY_SUMMARY_TIMEOUT", "120")),
    ...
    async def generate_weekly_forecast(
        ...
        """
        生成某区未来一周天气预报 Word 文档，并在聊天中提供下载。
        ...
        try:
            ...
            # 一周 = hour<=168，最多 14 个时次（7 天 × 白天/夜间）
            forecast_data = [d for d in data_list if d["hour"] <= 168][:14]
            if len(forecast_data) < 14:
                return json.dumps(
                    {
                        "error": (
                            f"预报数据不足 14 个时次（实际 {len(forecast_data)} 个），"
                            f"无法生成未来一周预报。请确认 240h 预报产品是否完整。"
                        )
                    },
                    ensure_ascii=False,
                )
        ...
        filename = f"{display_name}未来一周天气预报_{date_str}.docx"
        ...
        try:
            ...
            assistant_reply = (
                f"{display_name}未来一周天气预报文档已生成，点击下载：{download_md}"
            )
    ...
    def _fetch_latest_xml_from_ftp(self) -> str | None:
        ...
        try:
            ...
            print(f"[generate_weekly_forecast] FTP 拉取失败: {e}")
            return None
    ...
    def _extract_day_segments(self, paragraphs: list[str], need_count: int) -> list[dict]:
        ...
        跳过「三、未来八到十四天天气预报」等仍属预报正文的章节标题继续读（一般用不到，
        因为 need_count=14 通常在「未来一周」章节内就能凑够），
        ...
        if len(segments) < need_count:
            raise _BulletinParseError(
                f"「未来一周天气预报」章节合计只识别到 {len(segments)} 段日预报，"
    ...
    def _build_table(
        ...
        """模板字段（与 weather_templates/weekly/template.docx 对应）：
    ...
    async def _generate_summary(
        ...
        # 起报日 + 末日，便于在概述里写出「未来一周（X月X日至Y月Y日）」这种范围短语
        start_dt = base_time + timedelta(hours=12)
        end_dt = base_time + timedelta(hours=168)
        date_range_str = f"{start_dt.month}月{start_dt.day}日至{end_dt.month}月{end_dt.day}日"

        prompt = (
            f"你是资深气象预报员，为{area_label}撰写「未来一周天气预报」中的「天气综述」段落。\n"
            f"段落直接拼接在「一、天气综述」标题下，请直接给出正文，不要重复任何标题前缀。\n"
            f"要写成一段连贯、自然、有预报员口吻的中期天气趋势综述，"
            f"重点写晴雨大势、风力情况和气温变化，不要逐日罗列流水账。"
            f"只输出正文，单段不换行，70~120 字。\n\n"
            f"【未来一周天气过程事实（请据此归纳，不要逐条复述）】\n"
            ...
            f"【写作要求】\n"
            f"1. 开头建议为「预计未来一周（{date_range_str}）{area_label}以{sky_mood}为主」或类似句式，"
        ...
        def _fallback(reason: str, exc: BaseException | None = None) -> str:
            ...
            if exc is not None:
                log.warning(
                    "weekly_forecast summary fallback (%s): %s: %s",
                    reason, type(exc).__name__, exc,
                )
            else:
                log.warning("weekly_forecast summary fallback (%s)", reason)
        ...
        try:
            ...
            if requested:
                if requested in models:
                    ...
                    log.warning(
                        "weekly_forecast: summary_model=%r 不在 app.state.MODELS 中"
                        "（已知 %d 个），忽略该配置",
            ...
            if not model_id and current_model and current_model.get("id"):
                ...
                if cid in models:
                    ...
                    log.info(
                        "weekly_forecast: 当前对话模型 %r 不在后端代理可见列表中"
                        "（共 %d 个），将自动改用其它可用模型",
                        cid, len(models),
                    )
            if not model_id:
            ...
            log.info("weekly_forecast: 使用模型 %r 生成概况（候选 %d 个）", model_id, len(models))
    ...
    def _fallback_summary(self, area_label: str, stats: dict, date_range_str: str = "") -> str:
        ...
        if density == "none":
            head = f"预计未来一周{range_prefix}{area_label}以{sky_mood}为主"
        elif density == "frequent":
            day_hint = "、".join(days) if 0 < len(days) <= 4 else "部分时段"
            head = f"预计未来一周{range_prefix}{area_label}以{sky_mood}为主，{day_hint}有{kind_word}"
        else:
            day_hint = "、".join(days) if 0 < len(days) <= 4 else "部分时段"
            head = f"预计未来一周{range_prefix}{area_label}以{sky_mood}为主，{day_hint}有分散性{kind_word}"
