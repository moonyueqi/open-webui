"""
title: 未来三天天气预报生成器
description: 根据「北京市气象台 240h 精细化预报产品」自动生成某区未来三天天气预报 Word 文档
...
class Tools:
    ...
    def _build_table(
        ...
        按预报数据原始顺序，每个时次产出 1 行（扁平结构），用于模板按行循环：
          [
            {"date": "11日", "period": "白天", "weather": "晴",
             "wdir": "南风", "wspeed": "2级", "t_max": "31", "t_min": "-"},
            {"date": "11日", "period": "夜间", ...},
            ...
          ]
        ...
        for item in forecast_data:
            ...
            is_max = self._is_max_temp(base_hour, hour)
            rows.append(
                {
                    "date": date_key,
                    "period": period,
                    ...
                    "wspeed": item["wspeed"],
                    "t_max": item["t"] if is_max else "-",
                    "t_min": item["t"] if not is_max else "-",
                }
            )
        return rows
