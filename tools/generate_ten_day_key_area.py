"""
title: 未来十天气象服务专报生成器（首都关键区域）
description: 根据「北京市气象台 240h 精细化预报产品」自动生成首都关键区域气象服务保障中心
             所辖东城区 / 西城区 / 天安门地区的「未来十天气象服务专报」Word 文档
author: weather-tools
version: 1.0.0
license: MIT

【与 generate_three_day_key_area.py 的区别】
  1. 时效不同：拉取 240h 预报全部 20 个时次（10 天 × 白天/夜间），表格 20 行；
  2. 概述措辞写「预计未来十天…」，允许提及「前期、后期、过程」等长跨度表达；
  3. 模板与文件名指向 ten_day_key_area，与三天版本互不冲突。
其余（数据源、归一化逻辑、合并日期单元格、能见度留空、LLM+本地兜底）完全一致。
...
class Tools:
    class Valves(BaseModel):
        ...
        template_path: str = Field(
            default=os.environ.get(
                "WEATHER_TEMPLATE_TEN_DAY_KEY_AREA",
                "/app/weather_templates/ten_day_key_area/template.docx",
            ),
            description="未来十天气象服务专报（首都关键区域）docx 模板的绝对路径",
        )
        ...
        summary_timeout: int = Field(
            default=int(os.environ.get("TEN_DAY_KEY_AREA_SUMMARY_TIMEOUT", "120")),
    ...
    async def generate_ten_day_key_area_forecast(
        self,
        area: str,
        __user__: dict = None,
        ...
        if __event_emitter__:
            await __event_emitter__(
                {
                    "type": "status",
                    "data": {"description": "正在读取预报数据...", "done": False},
                }
            )
            ...
            return json.dumps(
                {
                    "error": (
                        ...
                        f"无法生成未来十天专报。请确认 240h 预报产品是否完整。"
                    )
                },
                ensure_ascii=False,
            )
        ...
        filename = f"未来十天气象服务专报-{AREA_DISPLAY_TITLE[area_key]}_{date_str}.docx"
        ...
        try:
            ...
            assistant_reply = (
                f"{AREA_DISPLAY_TITLE[area_key]}未来十天气象服务专报已生成，"
                f"点击下载：{download_md}"
            )
    ...
    # ---------- XML 取数（沿用 generate_three_day_forecast 的实现） ----------
    ...
    def _get_period_label(self, base_hour: int, hour: int) -> str:
        """白天/夜间判定（沿用 generate_three_day_forecast.py 的规则）。"""
        if hour == 12:
            if base_hour in (6, 9):
                return "白天"
    ...
    async def _generate_summary(
        ...
        first_date_label = date_labels[0] if date_labels else ""
        last_date_label = date_labels[-1] if date_labels else ""
        ...
        # 十天版本概述参考示例（东城-010311.docx）的风格：
        # 1) 写明「预计未来十天（X月X日至Y月Y日）」时段范围；
        # 2) 概括晴雨大势（可加一句能见度总体描述）；
        # 3) 若有显著大风，仅做事实陈述（不写"冷空气过程"等成因）；
        # 4) 末尾给出白天最高 / 夜间最低气温区间。
        # 注意：示例文里"冷空气活动""受冷空气影响"等表述是预报员人工背景判断，
        # 本工具没有冷空气/天气系统数据依据，**严禁**让 LLM 编造此类表述。
        prompt = (
            f"你是资深气象预报员，为首都关键区域气象服务保障中心撰写「未来十天{area_title}天气服务专报」"
            f"中的「天气综述」段落。\n"
            ...
            f"【写作要求】\n"
            f"1. 开头建议为「预计未来十天（{date_range_str}）{area_title}以{sky_mood}为主」或类似句式，"
            f"晴雨词只能用「{sky_mood}」，不要在其后加「天气」二字；\n"
            f"2. 接着可笼统概括能见度（如「能见度总体较好」），"
            f"若有显著大风过程（4 级以上偏北风），仅可做事实陈述，如"
        ...
        def _fallback(reason: str, exc: BaseException | None = None) -> str:
            fb = self._fallback_summary(area_key, stats, date_range_str)
            if exc is not None:
                log.warning(
                    "ten_day_key_area summary fallback (%s): %s: %s",
                    reason, type(exc).__name__, exc,
                )
            else:
                log.warning("ten_day_key_area summary fallback (%s)", reason)
        ...
        try:
            ...
            if requested:
                if requested in models:
                    ...
                    log.warning(
                        "ten_day_key_area: summary_model=%r 不在 app.state.MODELS 中"
                        "（已知 %d 个），忽略该配置",
            ...
            if not model_id and current_model and current_model.get("id"):
                ...
                if cid in models:
                    ...
                    log.info(
                        "ten_day_key_area: 当前对话模型 %r 不在后端代理可见列表中"
                        "（共 %d 个），将自动改用其它可用模型",
                        cid, len(models),
                    )
            if not model_id:
            ...
            log.info("ten_day_key_area: 使用模型 %r 生成概况（候选 %d 个）", model_id, len(models))
            ...
            if content:
                ...
                if self._validate_summary(cleaned, stats, date_labels):
                    return cleaned
                log.warning(
                    "ten_day_key_area summary failed fact-check, fallback used. raw=%r",
                    cleaned,
                )
                return self._fallback_summary(area_key, stats, date_range_str)
            return _fallback("模型返回内容为空")
        except Exception as e:
            log.exception("generate_ten_day_key_area summary failed: %s", e)
            return _fallback("模型调用异常", e)

    def _fallback_summary(self, area_key: str, stats: dict, date_range_str: str = "") -> str:
        """本地兜底：参考示例文本风格——
          「预计未来十天（X月X日至Y月Y日），<area>以XX为主，能见度总体较好[，受冷空气影响…]。
        ...
        # 主体描述
        if density == "none":
            head = f"预计未来十天{range_prefix}{area_title}以{sky_mood}为主，能见度总体较好"
        elif density == "frequent":
            day_hint = "、".join(days) if 0 < len(days) <= 4 else "部分时段"
            head = f"预计未来十天{range_prefix}{area_title}以{sky_mood}为主，{day_hint}有{kind_word}"
        else:
            day_hint = "、".join(days) if 0 < len(days) <= 4 else "部分时段"
            head = f"预计未来十天{range_prefix}{area_title}以{sky_mood}为主，{day_hint}有分散性{kind_word}"
    ...
    # ---------- 概述校验（沿用三天，去掉「正文禁出现气温」这条，因示例允许有气温） ----------
