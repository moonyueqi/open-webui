...
<Modal size="sm" bind:show>
	<div>
		<div class="flex items-center justify-between px-6 pt-5 pb-4">
			<div class="flex items-center gap-2.5">
				...
				>
					<svg
						...
						viewBox="0 0 20 20"
						fill="currentColor"
						class="size-[18px] text-gray-700 dark:text-gray-300"
					>
						<path
							fill-rule="evenodd"
			...
			<button
				class="rounded-md p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors"
				on:click={() => {
					show = false;
				}}
			>
		...
		<div class="px-6 py-5 dark:text-gray-200">
			<form
			...
			>
				<div class="max-h-[60vh] overflow-y-auto scrollbar-thin pr-1 -mr-1">
					{#if !loading}
				...
				<div
					class="flex justify-end pt-5 mt-5 border-t border-gray-100 dark:border-gray-800"
				>
					<button
						class="inline-flex items-center gap-1.5 px-5 py-2 text-sm font-medium bg-black hover:bg-gray-800 text-white dark:bg-white dark:text-black dark:hover:bg-gray-200 transition-colors rounded-lg disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
						type="submit"
						disabled={saving}
					>
						{#if saving}
