<script lang="ts">
	...
	const getCategoryList = async () => {
		loading = true;
		try {
			const res = await getSkillCategories(localStorage.token, page, viewOption, query).catch(
				(error) => {
					toast.error(`${error}`);
					return null;
				}
	...
	const deleteHandler = async (category: any) => {
		const res = await deleteSkillCategoryById(localStorage.token, category.id).catch((err) => {
			toast.error(`${err}`);
			return null;
		});
...
{#if loaded}
	...
	>
		...
		<div class="text-xs text-red-500 mt-2">
			{$i18n.t('All skills under this category will also be deleted.')}
		</div>
	</DeleteConfirmDialog>
		...
		<div class="flex flex-col gap-2 px-1 mt-1.5 mb-4 shrink-0">
			<div class="flex justify-between items-center">
				<div class="flex items-center gap-3 shrink-0">
					<div
						class="flex items-center justify-center w-9 h-9 rounded-xl bg-amber-500/10 dark:bg-amber-500/15"
					>
						<svg
				...
				<div class="flex w-full justify-end gap-1.5">
					{#if $user?.role === 'admin' || $user?.permissions?.workspace?.skills}
						<a
							...
							href="/workspace/skills/categories/create"
						>
					...
					<div class="gap-2.5 grid lg:grid-cols-2">
						{#each categories as category (category.id)}
							<a
								...
								href={`/workspace/skills/categories/${category.id}`}
							>
								<div class="flex items-start gap-3 flex-1 min-w-0">
									...
									<div class="flex-1 min-w-0">
										...
										>
											...
											<span class="text-gray-300 dark:text-gray-600">·</span>
											<span class="shrink-0">
												{$i18n.t('{{count}} skill(s)', { count: category.skill_count ?? 0 })}
											</span>
										</div>
									</div>
								</div>
