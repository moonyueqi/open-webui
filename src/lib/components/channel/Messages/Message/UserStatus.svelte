...
{#if user}
	...
	{#if (user?.groups ?? []).length > 0}
		<div class="mx-3.5 mt-2 flex flex-wrap gap-0.5">
			{#each user.groups as group}
				<Tooltip content={group.name}>
					<div
						class="px-1.5 py-0.5 rounded-lg bg-gray-50 dark:text-white dark:bg-gray-900/50 text-black transition text-xs max-w-[10rem] truncate"
					>
						{group.name}
					</div>
				</Tooltip>
