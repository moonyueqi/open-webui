...
class RadarPushWorker:
    ...
    def scan_once(self):
        ...
        # 仅当 _process_timestamp 返回 True（完整处理结束）才标记已处理；
        # 返回 False（数据未就绪）或抛异常时不标记，下一轮重试。
        try:
    ...
    def _process_timestamp(self, rf) -> bool:
        ...
        # TreRef 缺失：没有外推无法判断移动/趋势/是否入市，等待落盘后重试
        if not has_treref(self.ds, rf):
            log.info("TreRef 缺失：%s，等待外推数据，本轮跳过（不推送）", rf.treref_rel_path)
            return False

        # refl_raw：无 ET 过滤，用于"雷达监测"段描述实况强回波分布。
        refl_raw = grid.refl.copy()
        # refl_strong：抹掉顶高不足（ET<et_min_km）的格点，用于推送决策与强对流判别。
        ...
        # 有效强回波掩膜：四连通且≥min_points 的连片格点，实况判断统一基于此
        ...
        # "雷达监测"段：取无 ET 过滤的全局最强块（雷达上真正最强、不受顶高限制）
        ...
        # 方向轨迹起点用实况主威胁团质心，把移动方向锚定到实况正在盯的同一团
        ...
        # 风掣就绪门槛：须在任何去重决策之前判断，未就绪 return False 才不会推进去重状态
        ...
        # decide_approach，避免里程碑/等级状态被提前推进、日后真逼近时漏掉首次推送。
        ...
        # affected_now（文案口径，无 ET 过滤）决定预报段列哪些区县/乡镇；
        # 推送决策另用 ET 口径（valid_strong），二者解耦。
        ...
        for code, dmask in masks.districts.items():
            ...
            # 推送口径：区县代表强度取第 min_points 高格点值（防单点杂波）
        ...
        # 预报段对流类型用实况 Z + 实况 ET 判别：团在市内用全市内、未入市用主威胁团；
        # Z 取代表值（第 min_points 高格点）防单点杂波，ET 取范围内最大。
        cur_in_city = False
        if det.clusters:
        ...
        # 正文用模板原文，不做整段 LLM 润色（如需恢复调用 self._maybe_polish）
        text = template_text
        ...
        # 上传推送结果到 FTP，与企业微信解耦：失败只记日志、不影响主流程
        if self.ftp.enabled:
    ...
    def _build_realtime_line(
        ...
        echo_centroid 为回波主威胁团质心 (lat, lon)，direction=少动时按回波位置定上游侧。
        """
    ...
    def _build_fengche_line(self, observe_time: datetime) -> tuple[str, bool]:
        ...
        ready=True：本时次风掣已定稿（line 非空为文案，line 为空表示放弃、整行省略）。
        ready=False：文件未就绪且未超时，调用方应 return False、下一轮重试。

        等待判定：距观测时刻已过 fc.wait_minutes 分钟仍未找到文件即超时放弃；
        ...
        try:
            ...
            # 传入 LLM：仅在固定明细前插入一句"降水概况"定性短语（不含数值），数值明细
            # 由代码原样拼接。风掣行不交给 _maybe_polish 改写，避免固定明细被二次改动。
            line, rain_districts, gust_districts = build_rainfall_forecast_line(
                tp_grid=tp_grid,
                masks=fc_masks,
                start_hour=start_hour,
                end_hour=end_hour,
                llm=self.settings.llm if self.settings.llm.enabled else None,
                gust_report_min_level=fc.gust_report_min_level,
            )
            log.info("风掣预报行（lead=%dh, %d个区县有雨, %d个区县达阵风门槛）：%s",
                     found.lead_hour, len(rain_districts), len(gust_districts), line)
            self._record_fengche(
                ok=True, target_span=span,
                issue_time=found.issue_time.isoformat(),
                lead_hour=found.lead_hour,
                rain_districts=len(rain_districts),
                gust_districts=len(gust_districts),
