...
class GeoRegistry:
    ...
    @property
    def city_centroid(self) -> Tuple[float, float]:
        """苏州市域质心 (lat, lon)，用于站点方位/上游侧判定。"""
        c = self.city_geom.centroid
        return (float(c.y), float(c.x))

    def locate_point(self, lon: float, lat: float) -> Tuple[bool, bool, Optional[str]]:
        """判定一个经纬度点的归属。

        返回 (in_city, in_buffer, upstream_city_name)：
          · in_city：是否落在苏州市域内；
          · in_buffer：是否落在「市界+50km」检测缓冲区内（含市内）；
          · upstream_city_name：若点在市外且落入某个 upstream_cities 多边形，返回该城市名，否则 None。
        点完全在缓冲区外时 (False, False, None)。
        """
        pt = Point(float(lon), float(lat))
        in_buffer = bool(self._prep_buffer.intersects(pt))
        if not in_buffer:
            return (False, False, None)
        in_city = bool(self._prep_city.intersects(pt))
        if in_city:
            return (True, True, None)
        for name, _geom, pup in self.upstream_cities:
