...
_SYSTEM_PROMPT = (
    ...
    "3c. 「风掣预报：」开头那一行及其后逐区罗列的明细行（形如\"某区：最大小时雨强…毫米，"
    "最大阵风…级\"）必须整段原样保留、一字不改（该段已由系统定稿），不得改写、不得调整措辞或"
    "数值、不得合并或增删其中任何文字与换行。\n"
    ...
    "7. 保持初稿的分段结构：标题【…】、\"雷达监测：\"、\"实况提醒：\"、\"趋势预测：\"、\"风掣预报：\"及其"
    "后逐区明细各占一行；若初稿含\"具体影响乡镇如下\"段落，必须原样保留该段每一行（区县名与其"
    "乡镇列表不得改动），用换行分隔，不得合并成一段。"
)
...
# 风掣预报段数字密集（各区小时雨强+极大风级），校验时单独剥离、用宽松口径核对。
# 该段为「风掣预报：」首行 + 其后逐区明细行，作为一整块处理。
_FENGCHE_PREFIX = "风掣预报"


def _split_fengche(text: str) -> tuple[str, str]:
    """把文本按行拆成 (非风掣预报部分, 风掣预报段)。无该段则后者为空串。

    识别以「风掣预报」（兼容行首 emoji/符号前缀）为首行、直至文本末尾的连续块为
    风掣预报段（首行概况 + 逐区明细）。若未出现该前缀则整体归入非风掣预报部分。
    """
    lines = text.splitlines()
    start = next((i for i, ln in enumerate(lines) if _FENGCHE_PREFIX in ln), None)
    if start is None:
        return text, ""
    others = lines[:start]
    fengche = lines[start:]
