...
def _track_strongest_centroids(
    ...
    current_centroid 为实况主威胁团质心，给定时作为轨迹起点（frame_idx=-1）以锚定同一团；
    若与外推首团跳变 > max_jump_deg（疑似非同一团），则丢弃实况起点回退纯外推。
    """
...
class ExtrapResult:
    ...
    # 主威胁团质心轨迹首/末点 (lat, lon)，供画移动方向箭头；轨迹不足或"少动"时为 None。
...
def _affected_in_frame(
    ...
    外推采用宽松口径：单格点达标即计入，不做四连通过滤（实况判断仍用四连通，见 main.py）。
    """
    affected: Dict[str, Set[str]] = {}
...
def analyze_extrapolation(
    ...
    *,
    ...
    # 用主威胁团轨迹首末位移定方向，避免多回波时最强块跳变产生伪移动。
    direction = "少动"
    track_start: Optional[Tuple[float, float]] = None
    track_end: Optional[Tuple[float, float]] = None
