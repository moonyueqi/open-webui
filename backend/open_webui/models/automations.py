...
class AutomationTable:
    ...
    async def get_active_by_user(self, user_id: str) -> list[AutomationModel]:
        """Active automations for a user (used by Calendar for RRULE expansion).

        Calendar code is async; this method runs the sync session in a thread.
        """
        import asyncio
...
class AutomationRunTable:
    ...
    async def get_runs_by_user_range(
        ...
        """Past runs in [start_ns, end_ns) for a user, joined with parent automation.
