...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    METEO_SYSTEM_PROMPT = (
        ...
        "【技能与上下文使用】\n"
        "- 当系统消息中出现 <skill name=\"...\">...</skill> 块时，其中的内容是用户为本次对话挂载的"
        "知识或操作指南，应作为权威的事实/规则直接采用，无需质疑其相关性。\n"
        "- 当系统消息中出现 <available_skills>...</available_skills> 列表时，仅供你了解可用技能，"
        "若需要使用对应技能内容请提示用户显式调用。\n"
        "- 技能内容可能与气象领域无关（例如个人信息、历史记录、操作流程等），"
        "此时不要以\u201c超出气象服务范围\u201d为由拒答，而应基于技能中的事实直接给出回答，"
        "并在保持气象助手身份的前提下自然作答。"
    )
    messages = form_data.get("messages", [])
