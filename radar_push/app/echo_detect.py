...
def robust_value(values: np.ndarray, min_points: int) -> float:
    ...
    等价于"至少有 min_points 个格点达到的强度"。不足 min_points 个有效值时，
    取最小有效值；全为无效时返回 nan。
    """
...
def detect_clusters(
    ...
    *,
    ...
    for lab in range(1, n + 1):
        ...
        # 代表强度：块内第 min_points 高的格点值。等价于"至少有 min_points 个
        # 格点达到该强度"，可避免单个孤立尖峰（多为杂波）把整块顶成高等级。
    ...
    # 排序：先比代表强度，再比单点峰值，最后比块面积（格点数），消除"按位置先后"的偶然性。
