...
def _resolve_from_bearing(
    ...
    优先级：
      1. direction 为有效八方位 → 来向 = 去向反方向（回波向东移 → 来向西）。
      2. direction 少动/未知，但有回波团质心 → 来向 = 回波团相对市质心的方位
         （回波盘踞在哪一侧，就取哪一侧的上游站；如少动团在市西 → 取西侧）。
      3. 都没有 → None（保底全取，不漏报）。
    """
