...
class FengcheSettings:
    """风掣 AI 降水预报数据源（FTP）配置。

    风掣按整点逐小时起报，每份文件覆盖未来 1~24 小时逐时多要素（含 tp 逐小时降水）。
    本服务在推送时按「逐小时为界」选定目标时段，统计苏州各区县内 tp 范围拼成「风掣预报」行。
    数据源支持两种：local（部署服务器本地目录，现行方式）与 ftp（旧的独立 FTP 服务器）。
    """
    enabled: bool
    ...
    boundary_minute: int
    # 风掣等待超时（分钟）：推送时若风掣文件尚未就绪，则该时次不推、下轮重试，直到
    #   距观测时刻超过此分钟数仍未就绪才放弃（放弃后风掣行整行省略、不显示）。
    #   设 0 表示不等待（找不到即放弃，行省略），与"纯解耦"行为一致。
    wait_minutes: int
...
class ObsSettings:
    ...
    实况文件为某一时刻的全省站点快照，文件名时间戳为 UTC，「小时降水量」记录该时刻
    过去 1 小时累计雨量。本服务在推送时按观测时刻对齐出实况窗口，从对应文件里取
    苏州市内/上游侧的最大小时雨强与最大阵风，拼成「实况提醒」行。
    """
    enabled: bool
    ...
    # 阵风用列关键字（覆盖默认优先级），空则用 obs_reader 默认（最大阵风风速优先）
    gust_column: str
    # 触发「实况提醒」行的门槛
...
def load_settings() -> Settings:
    ...
    obs = ObsSettings(
        ...
        # 生产根目录形如 /bigdata/data/aws/raw，目录按 UTC 年/月/日 分层。
        local_base_dir=_env("OBS_LOCAL_BASE_DIR", str(BASE_DIR / "data_samples" / "raw"))
        or str(BASE_DIR / "data_samples" / "raw"),
        path_template=_env("OBS_PATH_TEMPLATE", "{Y}/{m}/{d}/SURF_GUST1MIN_{ymdHM}.xlsx")
        or "{Y}/{m}/{d}/SURF_GUST1MIN_{ymdHM}.xlsx",
        half_hour_boundary=_bool("OBS_HALF_HOUR_BOUNDARY", True),
